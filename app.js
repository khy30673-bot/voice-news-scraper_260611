...
(function () {
  ...
  var TTS = {
    ...
    init: function () {
      /* 단순화: 음성 목록을 미리 잡지 않는다.
         utterance.lang = 'ko-KR'만 설정하면 브라우저가 알아서 한국어 음성을 매칭한다.
         음성 목록을 명시적으로 voice 속성에 박지 않는 게 더 안정적이다. */
    },

    /* 호출되더라도 안전하도록 빈 함수로 유지 (호환용) */
    warmUp: function () {},

    /* text를 음성으로 읽고, 끝나면 onEnd 콜백 호출.
       단순한 흐름: 진행 중인 음성이 있을 때만 cancel, 그 외엔 곧바로 speak. */
    speak: function (text, onEnd) {
      ...
      /* 진행 중인 음성이 있을 때만 cancel — 불필요한 cancel은 일부 브라우저에서
         다음 speak를 깨뜨릴 수 있다. */
      if (ss.speaking || ss.pending) {
        ss.cancel();
        setTimeout(function () { self._doSpeak(text, onEnd); }, 130);
      } else {
        self._doSpeak(text, onEnd);
      }
    },

    _doSpeak: function (text, onEnd) {
      var u = new SpeechSynthesisUtterance(text);
      u.lang = 'ko-KR';
      u.rate = this.rate;
      /* voice 속성은 설정하지 않는다 — lang으로 브라우저가 알아서 매칭하게 둠.
         null이거나 잘못된 voice를 박으면 일부 환경에서 synthesis-failed가 발생한다. */
