...
(function () {
  ...
  var TTS = {
    ...
    rate: 0.95,
    voice: null,
    voicesReady: false,    // 음성 목록이 로드됐는지
    warmedUp: false,       // 모바일 음성 합성이 "깨어났는지"
    ...
    init: function () {
      ...
      /* 음성 목록을 즉시 시도. 모바일에서는 비어있을 가능성이 큼. */
      pickKoVoice();
      /* 음성 목록이 나중에 로드되면 이 이벤트로 다시 잡음. */
      window.speechSynthesis.onvoiceschanged = pickKoVoice;
    },

    /* 모바일 음성 합성 "워밍업" — iOS·일부 Android에서 첫 음성이 무음으로 끝나는 문제를 회피.
       사용자 첫 입력 이벤트 핸들러 안에서 호출돼야 효과가 있음.
       매우 짧은 빈 발화를 한 번 보내 음성 합성을 깨운다. */
    warmUp: function () {
    ...
    /* text를 음성으로 읽고, 끝나면 onEnd 콜백을 호출.
       ...
       모바일 호환을 위해 가능한 한 setTimeout 지연 없이 직접 호출한다. */
    speak: function (text, onEnd) {
      ...
      var self = this;
      var isSpeaking = window.speechSynthesis.speaking || window.speechSynthesis.pending;
      ...
      var doSpeak = function () {
        ...
        /* iOS 사파리 안전망: utterance를 외부 변수에 잠시 붙들어두지 않으면 GC돼서 음성이 잘림 */
        self._lastUtterance = u;
        window.speechSynthesis.speak(u);
      };

      if (isSpeaking) {
        /* 재생 중일 때만 cancel + 130ms 지연 (cancel 직후 곧바로 speak하면 일부 브라우저에서 씹힘) */
        window.speechSynthesis.cancel();
        setTimeout(doSpeak, 130);
      } else {
        /* 재생 중이 아니면 곧바로 speak — 사용자 입력 컨텍스트가 살아있어 iOS에서도 작동 */
        doSpeak();
      }
    }
  };
  ...
  function askWelcomeQuestion() {
    /* 환영 + 키 안내 + 권한 안내 (행동 지향적) + 스크린리더기 안내 */
    var welcome =
      '듣는 뉴스 스크랩에 오신 것을 환영합니다. ' +
      '스페이스 키로 다음으로 진행하시고, 백스페이스 키로 뒤로 돌아가실 수 있습니다. ' +
      '잠시 후 화면에 마이크 사용 권한 창이 뜹니다. ' +
      '스크린 리더기를 사용 중이시면 리더기 안내에 따라 허용을 선택해 주세요. ' +
      '키보드를 쓰신다면 탭 키로 허용 버튼에 이동한 뒤 엔터를 누르시면 됩니다.';
  ...
  var TODAY_NEWS_QUESTION =
    '오늘 날짜로 등록된 뉴스를 확인하시겠습니까? ' +
    '확인하시려면 마이크에 네 라고 답하시거나 키보드 스페이스 바를 눌러주세요.';
